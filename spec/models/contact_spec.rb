...
describe Contact do
  ...
  it { should belong_to(:imported_by).class_name('ContactImport') }
  ...
  describe "autocomplete title" do
    it "should query to autocomplete" do
      Contact.autocomplete_title("sam").to_sql.should == %{SELECT  distinct title FROM "entities"  WHERE "entities"."type" IN ('Contact') AND (LOWER(title) LIKE '%sam%') ORDER BY title ASC LIMIT 10}
    end
  end
