...
describe Deal do
  ...
  it { should ensure_inclusion_of(:status).in_array(%w(prospect qualify proposal negotiation won lost)) }

  it { should validate_presence_of(:value) }
  it { should ensure_inclusion_of(:value_type).in_array(%w(fixed hourly monthly yearly)) }
  it { should ensure_inclusion_of(:probability).in_range(0..100) }
